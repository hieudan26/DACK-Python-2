        ...
        if (self.checkTT() == True):
            if(self.checkGioiTinh() == True):
                if(self.checkIDNhanVien() == True):
                    ...
                    messagebox.showinfo("Add Nhân Viên", "Thêm nhân viên thành công", parent = self.AddForm)
                else:
                     messagebox.showerror("Add Nhân Viên", "Lỗi: ID nhân viên đã tồn tại", parent = self.AddForm)
            else:
                messagebox.showerror("Add Nhân Viên", "Lỗi: Giới tính chỉ chấp nhận 'Nam' hoac 'Nữ' ", parent = self.AddForm)
        else:
            messagebox.showerror("Add Nhân Viên", "Không được để trông các mục", parent = self.AddForm)
